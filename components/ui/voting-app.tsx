...
import { submitVote, getVotes, injectedConnector } from '@/web3/interactions'
...
export function VotingAppComponent() {
  ...
  const fetchVotes = useCallback(async () => {
    if (library && isCorrectNetwork) {
      try {
        const currentVotes = await getVotes(library)
  ...
  useEffect(() => {
    const checkNetwork = async () => {
      if (active && library) {
        ...
        setIsCorrectNetwork(network.chainId === 23295) // Updated to Sapphire testnet
        if (network.chainId !== 23295) {
          setNetworkError('Please connect to the Sapphire testnet (Chain ID: 23295)')
        } else {
          setNetworkError(null)
          fetchVotes() // Fetch votes when connected to the correct network
        }
      }
    }

    checkNetwork()
  }, [active, library, chainId, fetchVotes])
  ...
  const handleSubmitVote = async () => {
    if (selectedOption && library && isCorrectNetwork) {
      setIsVoting(true)
      try {
        const success = await submitVote(library, selectedOption)
        if (success) {
          await fetchVotes() // Fetch updated votes after successful submission
        ...
        <div className="md:flex">
          <div className="p-8 w-full">
            ...
            <Button
              ...
              disabled={!active || !selectedOption || isVoting || !isCorrectNetwork}
